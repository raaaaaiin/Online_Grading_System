...
<form method="POST"action="logout.php" >
			...
			<form method=POST action="adchangepass.php">
...
if (isset($_POST['submit']))
{
    ...
    $result = mysqli_query($config, "SELECT *from tbl_sepi_account WHERE Acc_ID='" . $userid . "'");
    $row = mysqli_fetch_array($result);
    if ($_POST["currentPassword"] == $row["Password"] && $_POST["newPassword"] == $_POST["confirmPassword"]) {
		
        mysqli_query($config, "UPDATE tbl_sepi_account set Password='" . $_POST["newPassword"] . "' WHERE Acc_ID='" . $userid . "'");
