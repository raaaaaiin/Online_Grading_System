...
if(isset($_POST['submit'])) {
    $selected_teachers = $_POST['teacher'];
    $email_list = array();
    foreach($selected_teachers as $teacher) {
        $query = "SELECT EMAIL FROM tbl_teacherinfo WHERE TID = $teacher";
        $result = mysqli_query($config, $query);
        $row = mysqli_fetch_assoc($result);
        $email_list[] = $row['EMAIL'];
    }
    $email_list_str = implode(",", $email_list);
    header("Location: send.php?emails=$email_list_str");
    exit;
}
...
<form method=POST action="teacherviewall.php">
    <input type=text name=search class="studenttxtviewall">
    <input type=submit name=sub class="studentbtnviewall">    
	
</form>
	<form method='post' action='index.php'>
	<input type="submit" name="email" value="SEND EMAIL" class="returnviewallbtn" style="
    margin-left: 43%; width: 127px;
">             
    ...
    if($result -> num_rows > 0){
        ...
        while($row = $result -> fetch_assoc()){
            ...
            echo "<td class=teacherinfo><input type='checkbox' name='teacher[]' value='".$row['EMAIL']."'>";
