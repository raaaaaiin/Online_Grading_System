...
if(isset($_POST['save'])){
    ...
    foreach ($row_data as $row) {
      ...
      if (@$check_result->num_rows > 0) {
          ...
          $update_query = "UPDATE tbl_grades SET Subject_Code = '$Subject_Code', Teacher_Code = 'Admin', $grade_column = '$Grade' WHERE Student_Code = '$Student_Code' AND SY = '$SY'";
          $result = $config->query($update_query);
      } else {
          // Insert a new record
          $insert_query = "INSERT INTO tbl_grades (Subject_Code, Teacher_Code, Student_Code, $grade_column, SY) VALUES ('$Subject_Code', 'Admin', '$Student_Code', '$Grade', '$SY')";
...
const gradeQuery = 'SELECT DISTINCT Grade_Level FROM tbl_subject';
...
function populateSectionDropdown(selectedGrade) {
 ...
 WHERE tbl_restriction.Grade_Level = '${selectedGrade}'
`;
console.log(query);
