...
$sql1 = "Insert into tbl_audithistory (e_name,e_action,e_date) values ('$name','Viewing Admin Audit Records',NOW())";
...
if (isset($_POST['search'])) {
	...
	if (!empty($_POST['dpa']) && !empty($_POST['dpb'])) {
		...
		if ($search != NULL) {
			...
			$query = "SELECT * FROM tbl_audithistory 
          WHERE (e_userID LIKE '%$search%' OR
			...
			$sql = "SELECT * FROM tbl_audithistory 
		...
		if ($search != NULL) {
			$sql = "Select * from tbl_audithistory where e_userID like '%$search%' or 
				e_name like '%$search%' or 
				e_action like '%$search%' or 
				e_date like '%$search%'";
		} else {
			$sql = "Select * from tbl_audithistory ORDER BY e_userID DESC LIMIT 100";
		}
